...
'''
data
'features': array([[array(['DenseHue.hvecs'], dtype='<U14'),
        array(['DenseSift.hvecs'], dtype='<U15'),
        array(['Gist.fvec'], dtype='<U9'),
        array(['Hsv.hvecs32'], dtype='<U11'),
        array(['Rgb.hvecs32'], dtype='<U11'),
        array(['Lab.hvecs32'], dtype='<U11')]], dtype=object),
...
def loadMfDIMvMlDataFromMat(mat_path, fold_mat_path,fold_idx=0):
    ...
    data = scipy.io.loadmat(mat_path)  # 完备数据
    datafold = scipy.io.loadmat(fold_mat_path)   # 不完备数据
    # multi-view data labels
    mv_data = data['X'][0]  # data[X][0][0].shape (4999, 100)
    labels = data['label']
    labels = labels.astype(np.float32)
    if labels.min() == -1:   # ？
        labels = (labels + 1) * 0.5
    ...
    folds_data = datafold['folds_data'] # 30个(4999, 6)  [0, 1, 1, 0, 1, 0]表明6个视图的存在与否？，最后表示标签，为0，表示全部mask？
    folds_label = datafold['folds_label']
    folds_sample_index = datafold['folds_sample_index']  # 30个4999，不同的fold 索引
    # incomplete data, label and random_sample index
    inc_view_indicator = np.array(folds_data[0, fold_idx], 'int32')  # TODO assert 每个fold_idx中有不同的mask？ (4999, 6)
    ...
    mv_data = [v_data[sample_index,:] for v, v_data in enumerate(mv_data)]  # 按索引提取多视图数据
    ...
    inc_mv_data = [np.random.randn(v_data.shape[0],v_data.shape[1]) for v_data in nor_mv_data] # 生成与视图格式一致的随机数, 作用？缺失数据位置被随机数占据
    for v,v_data in enumerate(nor_mv_data): # 缺失数据用随机数代替
        inc_mv_data[v][inc_view_indicator[:,v]==1,:] = v_data[inc_view_indicator[:,v]==1,:].copy() # 循环处理不同模态，挑选符合的部分数据，根据索引从完备数据制备不完备数据
...
class IncDataset(Dataset):
    def __init__(self, mat_path, fold_mat_path, training_ratio=0.7, val_ratio=0.15, fold_idx=0, mode='train',semisup=False):
        ...
        self.d_list = [da.shape[1] for da in inc_mv_data]  # 每个视图的维度
